 ...
 *
 * Creates Mutex, initializes nvs, loads op_settings from flash, initializes wifi, bluetooth, adc, i2c etc.
 * If pin 1 is enabled, enters config mode.
 * If wifi mode, tries to connect to wifi network. If connection is unsuccessful, enter config mode.
 ...
 *
 * This task is responsible for sending data to the client. It is notified by the acqAdc1Task when there is data to send.
 * If the client is BT or BLE it initializes the respective component.
 *
 ...
 *
 * This task is responsible for acquiring data from adc1. It is notified by the timerGrp0Isr when to acquire data.
 ...
 *
 * This task is responsible for acquiring data from adcExt. It is notified by the timerGrp1Isr when to acquire data.
 * It notifies the sendTask when there is data to send.
 ...
 *
 * This task is responsible for acquiring data from adc2 (battery). It is notified by the timerGrp1Isr when to acquire data.
 * It notifies the sendTask when there is data to send.
 * It is also the main task of CPU0 (PRO CPU)
 */
void IRAM_ATTR AbatTask(void) {
 ...
 *
 * This task is responsible for acquiring data from the I2C sensor. It is notified by the timerGrp1Isr when to acquire data.
void acqI2cTask(void)
{
    //uint16_t heart_rate;
    //uint16_t oxygen;
