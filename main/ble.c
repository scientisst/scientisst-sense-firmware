...
// static uint8_t indicate_data[GATTS_NOTIFY_LEN] = {0x01, 0x02, 0x03, 0x04,
...
static esp_ble_adv_data_t adv_data = {
    ...
    .max_interval = 0x000C,  // slave connection max interval, Time =
...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event,
    ...
    switch (event) {
        ...
        case ESP_GATTS_WRITE_EVT:
            /*
            DEBUG_PRINT_I(GATTS_TAG, "GATT_WRITE_EVT, conn_id %d, trans_id %d,
            handle %d", param->write.conn_id, param->write.trans_id,
            param->write.handle); if(!param->write.is_prep){
                DEBUG_PRINT_I(GATTS_TAG, "GATT_WRITE_EVT, value len %d, value
            ...
            gl_profile_tab[PROFILE_A_APP_ID].char_handle, sizeof(indicate_data),
            indicate_data, true);
                            }
                        }
                    }
        ...
        case ESP_GATTS_MTU_EVT:
            DEBUG_PRINT_I(GATTS_TAG, "ESP_GATTS_MTU_EVT, MTU %d",
                          param->mtu.mtu);
            break;
        ...
        case ESP_GATTS_CREATE_EVT:
            ...
            if (add_char_ret) {
                DEBUG_PRINT_E(GATTS_TAG, "add char failed, error code =%x",
                              add_char_ret);
            }
            break;
        ...
        case ESP_GATTS_ADD_CHAR_EVT: {
            ...
            if (add_descr_ret) {
                DEBUG_PRINT_E(GATTS_TAG,
                              "add char descr failed, error code =%x",
                              add_descr_ret);
            }
            break;
        }
        ...
        case ESP_GATTS_CONF_EVT:
            DEBUG_PRINT_I(GATTS_TAG, "ESP_GATTS_CONF_EVT, status %d",
                          param->conf.status);
        ...
        case ESP_GATTS_CONGEST_EVT:
            if (param->congest.congested == 0 && send_busy == SEND_AFTER_C0NG) {
                ...
                DEBUG_PRINT_W("gatts_profile_a_event_handler",
                              "ESP_SPP_CONG_EVT");
            }
            break;
        default:
...
void initBle(void) {
    ...
    if (ret) {
        DEBUG_PRINT_E(GATTS_TAG, "gatts app register error, error code = %x",
                      ret);
        exit(-1);
    }
    ...
    if (local_mtu_ret) {
        DEBUG_PRINT_E(GATTS_TAG, "set local  MTU failed, error code = %x",
                      local_mtu_ret);
    }
