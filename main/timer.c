 ...
 *
 * This function is called when the Task Watchdog Timer (TWDT) triggers. If bluetooth throughput is too high, either
 * acquring task (CPU1) or sending task (CP0) may starve other tasks of CPU. Hence, sampling rate or number of channels
 * need to be decreased. When that starvation happens, the Task Watchdog Timer (TWDT) will trigger and this handler will
