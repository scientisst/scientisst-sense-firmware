...
void IRAM_ATTR sendData(void) {
    ...
    // DEBUG_PRINT_I("sendData", "Data sent: %d bytes",
    // snd_buff_idx[bt_curr_buff]);
 ...
 */
static void IRAM_ATTR esp_spp_cb(esp_spp_cb_event_t event,
    ...
    switch (event) {
        case ESP_SPP_INIT_EVT:
            ...
            esp_spp_start_srv(ESP_SPP_SEC_AUTHENTICATE, ESP_SPP_ROLE_SLAVE, 0,
                              SPP_SERVER_NAME);
            ESP_LOGI("Init Bluetooth", "Device online with the name: %s",
                     device_name);
            break;
        case ESP_SPP_SRV_OPEN_EVT:  // Server connection open (first client
                                    // connection)
        ...
        case ESP_SPP_CLOSE_EVT:  // Client connection closed
            ...
            send_fd = 0;
            stopAcquisition();
            // Make sure that sendBtTask doesn't stay's stuck waiting for a
            // sucessful write
            break;
        ...
        case ESP_SPP_WRITE_EVT:  // write operation status changed
            ...
            if (param->write.status == ESP_SPP_SUCCESS) {
                ...
                // write failed because congestion, so wait event
                // 'ESP_SPP_CONG_EVT' and 'param->cong.cong == 0' to resend the
                // failed writing data.
            } else {
                // To resend the same buffer (note that because the write was
                // unsuccessful, bt_curr_buff wasn't updated)
                send_busy = SEND_AFTER_C0NG;
            }
            break;
        default:
    ...
    switch (event) {
        case ESP_BT_GAP_AUTH_CMPL_EVT: {
            if (param->auth_cmpl.stat == ESP_BT_STATUS_SUCCESS) {
                DEBUG_PRINT_I("esp_spp_cb", "authentication success: %s",
                              param->auth_cmpl.device_name);
                esp_log_buffer_hex(SPP_TAG, param->auth_cmpl.bda,
                                   ESP_BD_ADDR_LEN);
            } else {
                DEBUG_PRINT_E("esp_spp_cb", "authentication failed, status:%d",
                              param->auth_cmpl.stat);
            }
            break;
        }
        case ESP_BT_GAP_PIN_REQ_EVT: {
            DEBUG_PRINT_I("esp_spp_cb",
                          "ESP_BT_GAP_PIN_REQ_EVT min_16_digit:%d",
                          param->pin_req.min_16_digit);
            if (param->pin_req.min_16_digit) {
                DEBUG_PRINT_I("esp_spp_cb",
                              "Input pin code: 0000 0000 0000 0000");
        ...
        case ESP_BT_GAP_KEY_NOTIF_EVT:
            DEBUG_PRINT_I("esp_spp_cb", "ESP_BT_GAP_KEY_NOTIF_EVT passkey:%d",
...
void getDeviceName(void) {
    ...
    // Use esp_read_mac to infer the wifi station mac addr based in the base mac
    // addr read from EFUSE
    if (!strcmp(op_settings.com_mode, COM_MODE_TCP_AP)) {
        res = esp_read_mac(mac_iterface, ESP_MAC_WIFI_SOFTAP);
        // Use esp_read_mac to infer the wifi softap mac addr based in the base
        // mac addr read from EFUSE
        ...
        // Use esp_read_mac to infer the bluetooth mac addr based in the base
        // mac addr read from EFUSE
    } else {
    ...
    sprintf(mac_interface_str, "%x-%x-%x-%x-%x-%x", mac_iterface[0],
            mac_iterface[1], mac_iterface[2], mac_iterface[3], mac_iterface[4],
            mac_iterface[5]);
    sprintf(mac_bt_str, "%x-%x-%x-%x-%x-%x", mac_bt[0], mac_bt[1], mac_bt[2],
            mac_bt[3], mac_bt[4], mac_bt[5]);

    // Turn all characters of both mac address strings from lower case to upper
    // case
    ...
    sprintf(device_name, "%s-%s", BT_DEFAULT_DEVICE_NAME,
            (char*)(mac_bt_str + 12));
