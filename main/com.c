/** \file com.c
    \brief This file contains the functions that handle the communication with the client
 ...
 *
 * This function processes the command received from the client. It works in any mode and with all API's.
 *
 ...
 *
 * This function sets the output DAC level according to the received command. //TODO: Confirm this
 */
 ...
 *
 * This function changes the API mode and updates the functions that are used to acquire data and select channels.
...
uint8_t getPacketSize(void)
{
    ...
    {
        const uint8_t packet_size_num_chs[DEFAULT_ADC_CHANNELS + 1] = {0, 3, 4, 6, 7, 7,
                                                                       MAX_LIVE_MODE_PACKET_SIZE}; //Table that has the packet size in function of the number of channels
 ...
 *
 * This function starts the acquisition according to the received command. It starts the ADC and the timer.
 * It also sets the acquisition mode (live or simulated). It also changes the LED state.
 *
 * \param buff The received command
 * \param cmd The command (identifies if it is a live or a simulated acquisition)
 * \warning If changed, change same code in API (warning on identified on code)
 */
 ...
 * requests the firmware version packet. It fills one position of the send buffer with the information and marks it
 * as ready to be sent. It also changes the lowers the send threshold to 0, so that the data is sent as soon as possible
 * and then restores the send threshold to its previous value.
 */
