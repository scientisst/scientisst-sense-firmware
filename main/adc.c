...
void configAdc(int adc_index, int adc_resolution, int adc_channel)
{
    ...
    if (adc_index == 1)
    {
        adc1_config_channel_atten(adc_channel,
                                  ADC1_ATTENUATION);        //Atennuation to get a input voltage of 0 to 2.6V
    } else if (adc_index == 2)
    {
...
void acquireAdc1Channels(uint8_t *frame)
{
    ...
    for (i = 0; i < num_intern_active_chs; i++)
    {
        ...
        {
            adc_res[i] = adc1_get_raw(analog_channels[active_internal_chs[i]])
