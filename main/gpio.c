...
void gpioConfig(gpio_mode_t mode, gpio_int_type_t intr_type,
    ...
    if (pull_down_en && pull_up_en) {
        DEBUG_PRINT_E("gpioConfig",
                      "Pin cannot have both pull up and pull down enabled");
        return;
    }
...
void gpioInit(void) {
    ...
    gpioConfig(GPIO_MODE_OUTPUT, GPIO_PIN_INTR_DISABLE,
               ((1ULL << STATE_LED_R_IO) | (1ULL << BAT_LED_STATUS_IO) |
                (1ULL << O0_IO) | (1ULL << O1_IO) | (1ULL << SPI3_CS0_IO)),
               0, 0);
#else
    gpioConfig(GPIO_MODE_OUTPUT, GPIO_PIN_INTR_DISABLE,
               ((1ULL << STATE_LED_R_IO) | (1ULL << STATE_LED_G_IO) |
                (1ULL << STATE_LED_B_IO) | (1ULL << BAT_LED_STATUS_IO) |
                (1ULL << O0_IO) | (1ULL << O1_IO) | (1ULL << SPI3_CS0_IO)),
               0, 0);
#endif

    gpioConfig(GPIO_MODE_INPUT, GPIO_PIN_INTR_DISABLE,
               ((1ULL << I0_IO) | (1ULL << I1_IO)), 1,
               0);  // The 2 IO inputs
...
bool IRAM_ATTR gpioDrdyIsrHandler(void) {
    ...
    // terminated. vTaskNotifyGiveFromISR(acq_adc_ext_task, NULL); TODO: Check
    // if we can comment this
