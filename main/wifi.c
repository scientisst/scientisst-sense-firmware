...
#define KEY_SETTINGS_INFO \
    "opSettingsInfo"                   // key used in NVS for connection info
...
#define MDNS_HOST_NAME \
    "scientisst"  // Specify the domain name used in the mDNS service. Note that
                  // webpage also take it as a part of URL where it will send
...
int wifi_init_sta(void) {
    ...
    // Store
    //  If ssid is empty "", attempt to connect will not fail. This is not
    //  desirable
    ...
    if (bits & WIFI_CONNECTED_BIT) {
        DEBUG_PRINT_I("wifi station", "connected to ap SSID:%s password:%s",
                      (char*)wifi_config.sta.ssid,
                      (char*)wifi_config.sta.password);
        ret = 0;
    } else if (bits & WIFI_FAIL_BIT) {
        DEBUG_PRINT_W(
            "wifi station", "Failed to connect to SSID:%s, password:%s",
...
int getOpSettingsInfo(op_settings_info_t* _op_settings) {
    ...
    version, g_version); nvs_close(handle); return -1;
    }*/
...
void saveOpSettingsInfo(op_settings_info_t* pOpSettingsInfo) {
    ...
    ESP_ERROR_CHECK(nvs_set_blob(handle, KEY_SETTINGS_INFO, pOpSettingsInfo,
                                 sizeof(op_settings_info_t)));
...
int wifiInit(uint8_t force_ap) {
    ...
    ESP_ERROR_CHECK(mdns_service_add(
        "ESP32-WebServer", "_https", "_tcp", 443, serviceTxtData,
        sizeof(serviceTxtData) / sizeof(serviceTxtData[0])));
    netbiosns_init();
