...
static void write_frame_Scientisst(uint8_t *frame, const uint16_t *adc_internal_res,
...
static void write_frame_JSON(uint8_t *frame, const uint16_t *adc_internal_res,
                             const uint32_t *adc_external_res, const uint8_t io_state);
static inline void IRAM_ATTR write_frame(uint8_t *frame, const uint16_t *adc_internal_res,
                                         const uint32_t *adc_external_res, const uint8_t io_state);
...
_Noreturn void IRAM_ATTR task_acquisition(void *not_used)
{
    ...
    while (1)
    {
        ...
        CHECK_NUM_CHANNELS_IS_VALID(num_intern_active_chs, num_extern_active_chs);
        ...
        write_frame(frame_buffer[acq_curr_buff] + frame_buffer_write_idx[acq_curr_buff], adc_internal_res,
                    adc_external_res, io_state);
        ++crc_seq;                                            // Increment sequence number
        frame_buffer_write_idx[acq_curr_buff] += packet_size; // Update write index
        ...
        if (frame_buffer_write_idx[acq_curr_buff] >= send_threshold)
        {
            ...
            frame_buffer_ready_to_send[acq_curr_buff] = 1;
            ...
            while (frame_buffer_ready_to_send[acq_next_buff] == 1)
            {
                                      ...
                                      uint32_t *adc_external_res)
{
    ...
    *io_state |= (gpio_out_state[1] & 0b1) << 4;
    ...
    for (int i = 0; i < num_intern_active_chs; ++i)
    {
        adc_internal_res[i] = (uint16_t)adc1_get_raw(analog_channels[active_internal_chs[i]]);
    }
#endif
...
static inline void IRAM_ATTR write_frame(uint8_t *frame, const uint16_t *adc_internal_res,
                                         const uint32_t *adc_external_res, const uint8_t io_state)
{
    if (api_config.api_mode == API_MODE_SCIENTISST)
    {
                                             ...
                                             const uint32_t *adc_external_res, const uint8_t io_state)
{
    ...
    // Store seq number
    *(uint16_t *)(frame + packet_size - 2) = (uint16_t)(crc_seq << 4);
