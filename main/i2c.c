...
esp_err_t i2cMasterInit(void)
{
    ...
    conf.scl_pullup_en = GPIO_PULLUP_ENABLE;    /*!< Internal GPIO pull mode for I2C scl signal*/
 ...
 *
 * Implements the necessary configurations in order to read HR+SpO2. Based on page 13 of "Measuring Heart Rate
 * and SpO2Using the MAX32664A –   A Quick Start Guide"
 ...
 *
 * Reads the HR+SpO2 values from the MAX32664. Based on page 13 of "Measuring Heart Rate
 * and SpO2Using the MAX32664A –   A Quick Start Guide"
 * https://pdfserv.maximintegrated.com/en/an/ug7087-max32664a-quick-start-guide-rev-1-p1.pdf
 *
 * \param heart_rate Pointer to the variable where the heart rate value will be stored.
 * \param oxygen Pointer to the variable where the SpO2 value will be stored.
 * \param confidence Pointer to the variable where the confidence value will be stored.
 * \param status Pointer to the variable where the status value will be stored.
 *
 * \return:
 *    - 0 Success
 *    - 1 Error
...
uint8_t MAX32664_GetBPM(uint16_t *heart_rate, uint16_t *oxygen, uint8_t *confidence, uint8_t *status)
{
    ...
    /*if(MAX32664_SMBusRead(MAX32664_DEFAULT_ADDRESS, READ_DATA_OUTPUT, NUM_SAMPLES, 0, 0, &samples_ready, 1, CMD_DELAY)){
        DEBUG_PRINT_E("MAX32664_GetBPM", "Get number of bytes avaiable in FIFO failed");
        return 1;
    }
 ...
 * \param rcv_data_size Size of recieving data of interest. Normally the slave sends 1 byte for "Read status byte" followed by the data of interest
 * (Total recieved bytes should be always more than 1 byte -> status byte + data of interest)
 * \param delay_ms Delay in ms between the write and read commands.
 *
 * \return:
 *   - 0 Success
 *   - 1 Error
 ...
 * \param family_byte Family byte of the command.
 * \param index_byte Index byte of the command.
 * \param write_data Pointer to the buffer where the data to be written is stored.
 * \param write_data_size Size of the data to be written.
 * \param delay_ms Delay in ms between the write and read commands.
 *
 * \return:
 *   - 0 Success
                        ...
                        uint8_t write_data_size, uint8_t delay_ms)
{
    ...
    i2c_master_write_byte(cmd, family_byte, ACK_CHECK_EN);   //Send addr of register to be read
    i2c_master_write_byte(cmd, index_byte, ACK_CHECK_EN);    //Send addr of register to be read
    i2c_master_write(cmd, write_data, write_data_size, ACK_CHECK_EN);    //Send write_data_size bytes
    i2c_master_stop(cmd);
 ...
 * \param temp_amb Pointer to the variable where the ambient temperature will be stored.
 * \param temp_amb_int Pointer to the variable where the ambient temperature in integer format will be stored.
 *
 * \return:
 *   - 0 Success
 *   - 1 Error
 ...
 *
 * \param slave_addr Slave address of the MLX90614.
 * \param temp_obj Pointer to the variable where the object temperature will be stored.
 * \param temp_obj_int Pointer to the variable where the object temperature in integer format will be stored.
 *
 * \return:
 *   - 0 Success
 *   - 1 Error
