...
esp_err_t i2cMasterInit(void){
    ...
    return i2c_driver_install(i2c_master_port, conf.mode, I2C_MASTER_RX_BUF_DISABLE, I2C_MASTER_TX_BUF_DISABLE, 0);     //Master doesn't need RX and TX buffers
}

void MAX32664_Init(){
    gpioConfig(GPIO_MODE_OUTPUT, GPIO_PIN_INTR_DISABLE, ((1ULL<<MAX32664_MFIO_IO) | (1ULL<<MAX32664_RSTN_IO)), 0, 0);
...
/**
 * @brief Implements the necessary configurations in order to read HR+SpO2. Based on page 13 of
 * "Measuring Heart Rate and SpO2Using the MAX32664A –   A Quick Start Guide"
...
uint8_t MAX32664_GetBPM(uint16_t* heart_rate, uint16_t* oxygen, uint8_t* confidence, uint8_t* status){
    ...
    if(MAX32664_SMBusRead(MAX32664_DEFAULT_ADDRESS, READ_DATA_OUTPUT, READ_FIFO_DATA, 0, 0, data_buff, SIZE_SAMPLE, CMD_DELAY)){
